...
if (window.jQuery)(function ($) {
	...
	*/
	$.extend($.fn.MultiFile, {
		...
		data: function () {
			...
			if(!wp || !wp.length)
				return !console.error('데이터를 찾을 수 없습니다.');
		...
		files: function () {
			var mf = this.MultiFile('data');
			if(!mf) return !console.log('초기화되지 않았습니다');
			return mf.files || [];
		},
	...
	*/
	$.fn.MultiFile.options = { //$.extend($.fn.MultiFile, { options: {
		...
		STRING: {
			remove: 'X',
			denied: '이 파일의 확장자( $ext )는 업로드 할수없습니다.',
			file: '$file',
			selected: '선택한 파일은 : $file',
			duplicate: '이 파일은 이미 선택된 파일 입니다.:\n$file',
			toomuch: '선택한 파일의 크기가 초과 되었습니다. ($size)',
			toomany: '이미지는 최대  $max 까지 가능합니다.',
			toobig: '$file 이 파일은 너무 큽니다. (max $size)'
		},
